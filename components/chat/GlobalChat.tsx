...
export function GlobalChat() {
  ...
  // Carregar mensagens do localStorage no início
  useEffect(() => {
    const name = localStorage.getItem('sm_user_name') || 'Anónimo'
    setUserName(name)

    // Verificar se existe cooldown guardado (opcional, para persistir entre reloads)
    const lastSent = localStorage.getItem('sm_last_message_time')
    if (lastSent) {
    ...
    const savedMessages = localStorage.getItem('sm_chat_history')
    if (savedMessages) {
      try {
    ...
    channel.subscribe('message', (msg) => {
      setMessages((prev) => {
        const newMessages = [...prev, msg.data]
        const trimmedMessages = newMessages.slice(-200)
        localStorage.setItem('sm_chat_history', JSON.stringify(trimmedMessages))
  ...
  const sendMessage = (e: React.FormEvent) => {
    e.preventDefault()
    if (!input.trim() || !channelRef.current || cooldown > 0) return
    ...
    setInput('')
    setCooldown(30)
    localStorage.setItem('sm_last_message_time', Date.now().toString())
  }
      ...
      <div className="p-4 bg-white border-t border-gray-100">
        <form onSubmit={sendMessage} className="relative flex items-center gap-2">
          <input
            ...
            disabled={cooldown > 0}
            placeholder={cooldown > 0 ? `Aguarde ${cooldown}s...` : "Escreve uma mensagem..."}
            className="flex-1 bg-gray-50 border-none px-5 py-3.5 rounded-2xl outline-none focus:ring-2 focus:ring-red-100 transition-all text-sm text-gray-800 placeholder:text-gray-400 disabled:opacity-50"
          />
          <button 
            type="submit"
            disabled={!input.trim() || cooldown > 0}
            className="bg-[#bc2a24] text-white p-3 rounded-xl hover:scale-105 active:scale-95 transition-all shadow-lg shadow-red-200 disabled:bg-gray-200 disabled:text-gray-400 disabled:shadow-none min-w-[48px] flex items-center justify-center"
          >
            {cooldown > 0 ? (
