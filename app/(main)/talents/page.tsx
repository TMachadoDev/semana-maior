...
import { Clock, MapPin, Star, Music, Headphones, Users } from 'lucide-react'
...
export default function TalentsPage() {
  const { data: talents = [], isLoading, error, refetch } = useQuery({
    queryKey: ['talents'],
  ...
  return (
    <div className="min-h-screen bg-white pb-24">
      <PageHeader title="Talentos" subtitle="Shows & Atrações" showBack />
      ...
      {error && (
        <div className="px-5 py-10 text-center">
          <p className="text-sm text-gray-400">Não foi possível carregar os talentos.</p>
          <button onClick={() => refetch()} className="mt-3 text-[#bc2a24] text-sm font-medium">Tentar novamente</button>
        </div>
      )}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {(talent.performAt || talent.venue || talent.school) && (
                        <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                          ...
                          {talent.school && (
                            <span className="text-[10px] text-gray-400 flex items-center gap-1">
                              <Users className="w-2.5 h-2.5" />{talent.school.name}
                            </span>
                          )}
                        </div>
                      )}
